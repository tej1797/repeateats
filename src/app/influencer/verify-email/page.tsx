    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden"
      ...
      <div className="pointer-events-none absolute"
        style={{
          ...
          top: '20%', left: '50%', transform: 'translateX(-50%)', filter: 'blur(60px)',
        }} />

      <div className="relative w-full max-w-[420px] rounded-2xl p-8 text-center"
        style={{ background: '#fff', borderTop: '4px solid #7E22CE', boxShadow: '0 25px 60px rgba(0,0,0,0.45)' }}>

        <div className="font-display text-[24px] font-extrabold tracking-tight leading-none mb-6">
          Rep<span style={{ color: '#E85D04' }}>EAT</span>
          <span className="text-[15px] font-semibold text-gray-400 ml-2">Creator</span>
        </div>

        {/* Animated envelope */}
        <div className="relative w-24 h-24 mx-auto mb-6">
          <div className="w-full h-full rounded-2xl flex items-center justify-center text-5xl transition-all duration-500"
            style={{
              background: resent ? 'linear-gradient(135deg, #F5F3FF, #EDE9FE)' : '#F9FAFB',
              border: `2px solid ${resent ? '#C4B5FD' : '#E5E7EB'}`,
              boxShadow: resent ? '0 0 40px rgba(126,34,206,0.2)' : undefined,
            }}>
            {resent ? '📨' : '✉️'}
          </div>
          {resent && (
            <div className="absolute inset-0 rounded-2xl animate-ping"
              style={{ border: '2px solid rgba(126,34,206,0.3)' }} />
          )}
        </div>
        ...
        <p className="text-[13px] text-gray-400 mb-6">
          Click the link to activate your creator account and start finding collabs.
        </p>
        ...
        <button onClick={handleResend} disabled={cooldown > 0 || !email}
          ...
          style={{
            background: cooldown > 0 ? '#F9FAFB' : resent ? '#F5F3FF' : '#7E22CE',
            color: cooldown > 0 ? '#9CA3AF' : resent ? '#7E22CE' : '#fff',
        ...
        <div className="mt-5 rounded-xl p-4 text-left space-y-1.5"
          style={{ background: '#F9FAFB', border: '1px solid #F3F4F6' }}>
          <p className="text-[12px] font-semibold text-gray-500 mb-2">Can&apos;t find the email?</p>
          {['Check your spam or junk folder', 'Make sure you entered the right email', 'It can take a minute to arrive'].map((tip) => (
            <p key={tip} className="text-[12px] text-gray-400 flex items-start gap-2">
              <span style={{ color: '#7E22CE' }}>•</span> {tip}
            </p>
          ))}
        </div>

        <div className="mt-5 flex items-center justify-center gap-4 text-[13px]" style={{ color: '#9CA3AF' }}>
          <a href="/influencer" className="hover:underline hover:text-gray-600 transition-colors">Sign in</a>
          <span>·</span>
          <a href="/influencer/signup" className="hover:underline hover:text-gray-600 transition-colors">Different email</a>
          <span>·</span>
